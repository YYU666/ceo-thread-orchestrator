...
The driver freezes before dispatch when the Host-projected next request reaches the smaller of 90% of the configured context threshold or the model window minus reserved output. After two Host-confirmed compactions, the task runtime ledger sets `rotationRecommended` and `rotationNextAction=finish_current_slice_then_prepare_verified_takeover`. This is a scheduling signal, not evidence of current context exhaustion. Finish the bounded slice and prepare compact takeover; execute replacement only through the verified lifecycle when memory and Host are ready. Until then, permit safe work below the pressure limit. Compaction count remains task-scoped, monotonic, and source-backed. The old task stays permanently frozen after a real freeze or replacement. Clean takeover retains its 30000-token ceiling.
