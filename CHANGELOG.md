...
- Added the `ceoflow-budget-governor` OpenClaw runtime plugin and made it mandatory for paid writer dispatch. The bridge now verifies live hooks/Gateway methods, arms an immutable task/session budget, keeps the executor allowlist at `read/apply_patch/exec/process`, blocks excess tools, cancels runaway runs, and fails closed on missing telemetry.
- Closed the first live budget-fuse follow-up: OpenClaw 2026.7.1 can report its native 1M model/session window even when the dedicated executor declares 25k. The governor no longer confuses that window metadata with actual request input; it now records the propagation mismatch, checks assembled `llm_input` size before dispatch, verifies exact usage after output, and writes a complete terminal telemetry record immediately when aborting even if `agent_end` never arrives.
- Replaced advisory-only OpenClaw budgets with separate hard model-request, tool-call, per-result, cumulative tool-output, uncached-input, cached-input, cumulative-input, gross-session, and rolling-TPM limits. `providerCallCount=null` no longer passes validation; cache tokens count toward cumulative cost/rate protection without being misreported as one request's context occupancy.
- Made host tool trace authoritative for external command exit codes. Budget fuse results are terminal and never use the transient network retry path; no provider/model fallback is introduced.
- Made the one allowed transient retry consume the original task budget rather than resetting it: attempt-two arming subtracts prior model/tool/token/tool-output telemetry and blocks when prior telemetry is incomplete or the task remainder is exhausted.
- Classified explicit upstream capacity failures (`temporarily overloaded`, unavailable/busy/capacity errors, and HTTP 502/503/504) as bounded transient provider failures. They may receive the existing single 60-second unchanged-workspace retry, while auth, 429/rate-limit, quota, HTTP 500, fallback, and mutated-workspace cases remain fail-closed.
- Switched the cost-efficient OpenClaw default to `moonshot/kimi-k3` under a conservative Tier1 policy: dynamic `off/adaptive` thinking by task risk, no cross-provider fallback, at most three concurrent K3 tasks, and per-task request/cumulative/call/TPM ceilings kept far below the provider account limits.
- Fixed external model preflight for Agent-scoped credentials: exact pinned routes now verify the task's target OpenClaw Agent instead of incorrectly using the default `main` Agent's auth state.
- Closed the OpenClaw "minimal Agent in name only" gap: live preflight now verifies the dedicated executor's per-agent skill/tool/bootstrap budgets before contacting a model, and the canonical config fragment prevents global Skills and full tool schemas from inflating every task.
- Made external usage accounting cache-conservative by recording gross prompt, cached/uncached input, last-request input, total tokens, and provider-call count where available; receipts now fail validation on per-request, cumulative, or call-budget overrun.
- Replaced permanent OpenClaw project-role conversation reuse with stable logical lanes plus one clean physical session generation per bounded task. Terminal sessions are archived and never restored for follow-up work; Zhixia remains the durable history owner.
- Added compact `ProviderTaskView` rendering and hard context budgets for initial/per-request/cumulative input, provider calls, gross TPM, and tool output. Oversized context blocks before provider execution, and reported cumulative overrun fails receipt acceptance.
- Required a dedicated minimal `ceoflow-executor` OpenClaw Agent so fresh task sessions do not inherit the default personal Agent's unrelated Skills and bootstrap context.
- Extended task/session schemas, roster tracking, OpenClaw executor guidance, smoke prompts, and bridge tests for single-task archival and context-budget enforcement.
